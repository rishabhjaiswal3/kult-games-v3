...
export function InventoryStatsRail({
  ...
  return (
    <div
      className={cn(
        "arena-panel grid w-full grid-cols-2 gap-px overflow-hidden border-white/8 bg-[#04080f]/90 sm:grid-cols-4 lg:min-w-0 lg:flex-1",
        className
      )}
    >
      {stats.map((stat) => (
        <div
          key={stat.label}
          className="flex min-w-0 items-center gap-2.5 bg-[#04080f]/95 px-3 py-2.5 sm:px-3.5"
        >
          <div
            className="grid h-9 w-9 shrink-0 place-items-center rounded-md border border-white/8 bg-white/[0.03]"
            style={{ boxShadow: `0 0 14px ${stat.color}22` }}
          >
