...
export function GuessTheAiDetail({ game }: { game: Game }) {
  ...
  return (
    <div className="mx-auto w-full max-w-[1320px] pb-20 text-[#97979f] [&_p]:text-justify">
      ...
      <section className="relative overflow-hidden rounded-b-xl border border-purple-500/60 bg-[#050b14]">
      ...
      <section className="mt-14 grid items-end gap-8 lg:grid-cols-[minmax(0,1fr)_540px]">
        ...
        <div className="rounded-[13px] border border-[#24103e] bg-[#09051c] p-5"><div className="mb-5 flex items-center justify-between"><h2 className="font-tech text-xl font-bold uppercase text-white sm:text-2xl">Launch Control</h2><span className="rounded-md bg-white/10 px-5 py-1 text-white/35">v1.0</span></div><button type="button" onClick={play} className="flex w-full items-center justify-center gap-2 rounded-md bg-gradient-to-r from-[#681fd0] to-[#973af4] py-3 text-xl font-semibold text-white transition hover:brightness-110 active:translate-y-0"><Play className="h-6 w-6" /> Play</button><button type="button" onClick={() => navigate("/inventory")} className="mt-4 flex w-full items-center justify-center gap-2 rounded-md border border-[#25103d] bg-[#070214] py-3 text-xl font-semibold text-white transition hover:border-purple-600 active:translate-y-0"><BriefcaseBusiness className="h-6 w-6" /> Inventory</button></div>
      </section>
      ...
      ].map((item) => <button key={item.label} type="button" onClick={item.action} disabled={!item.action} className="flex items-center gap-4 rounded-[5px] border border-[#16132d] bg-[#080719] p-3 text-left transition enabled:hover:border-purple-400/50 disabled:cursor-default"><span className={`grid h-16 w-16 shrink-0 place-items-center rounded-[10px] border ${item.color}`}><item.icon className="h-8 w-8" /></span><span><span className="block text-xs text-white">{item.label}</span><span className="mt-1 block text-2xl text-white sm:text-[28px]">{item.value}</span></span></button>)}</section>
      ...
      <div className="mt-14 grid items-start gap-8 lg:grid-cols-[minmax(0,1fr)_350px] xl:grid-cols-[minmax(0,1fr)_390px]">
        ...
        <aside className="overflow-hidden rounded-[16px] border border-[#432460] bg-gradient-to-b from-[#110b22] to-[#050a13] shadow-[0_20px_70px_rgba(0,0,0,0.35)] lg:sticky lg:top-24 [&_p]:!text-left">
          <div className="border-b border-white/5 bg-[radial-gradient(circle_at_50%_0%,rgba(176,68,255,0.22),transparent_68%)] px-5 pb-5 pt-7 text-center"><img src={logo} alt="Guess the AI" className="mx-auto h-[112px] w-[118px] object-contain" /><h3 className="mt-1 font-tech text-lg font-bold uppercase text-white">Guess the AI</h3></div>
          <div className="p-5"><p className="text-[15px] leading-[1.7] text-[#a5a5ad]">Welcome to Guess The AI, where every image is a challenge. Test your ability to spot AI-generated visuals, compete across multiple game modes, earn rewards, and prove your skills against players from around the world.</p><div className="mt-4 flex items-center justify-between"><span className="rounded border border-emerald-700/70 bg-emerald-950/70 px-2.5 py-1 text-[10px] font-medium uppercase tracking-wider text-emerald-300">Puzzle</span><span className="text-[11px] uppercase tracking-wider text-white/35">Browser game</span></div><button type="button" onClick={play} className="mt-5 flex w-full items-center justify-center gap-2 rounded-md bg-gradient-to-r from-[#4c48ff] to-[#d33df0] py-3 font-semibold text-white transition hover:brightness-110 active:translate-y-0"><Play className="h-4 w-4" /> Play now</button><button type="button" onClick={share} className="mt-3 flex w-full items-center justify-center gap-2 rounded-md border border-[#42295f] bg-[#10091e] py-2.5 text-sm text-white transition-colors hover:border-purple-400/60 active:translate-y-0"><Share2 className="h-4 w-4" /> Share</button></div>
        </aside>
      </div>
    </div>
  );
